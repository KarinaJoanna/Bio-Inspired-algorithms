...
% Definir el ancho de banda de tono (bw) como la diferencia entre 
% los límites superior e inferior dividida por 1000
bw = (Ui - Li) / 1000;
%bw = 1;

% Definir el tamaño de la memoria de armonía (k) y el número máximo d
% e iteraciones (max_iter)
k = 20;
max_iter = 10000;
...
for g = 1:max_iter
    ...
    new_f = funcion_objetivo(newX,1);
    % Comparar la nueva solución con la mejor solución en la memoria de armonía actual
    [best_f,best_index] = max(funcion_objetivo(HM,k));
    if new_f < best_f
...
[best_fn, best_indexn] = min(funcion_objetivo(HM,k));
